...
$result = mysqli_query($db,'SELECT `userid` FROM `following` WHERE `followerid` = '.$_SESSION['id'].'');
//print_r($result);
$t_fol = 0;
if(mysqli_num_rows($result) > 0){
	while($fol_id = $result->fetch_object()->userid){
	...
	?>
    <script>
		document.getElementById("following").innerHTML = '<?php echo $t_fol; ?>';
	</script>
    <?php
}
?>